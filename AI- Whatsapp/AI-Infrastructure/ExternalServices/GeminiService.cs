...
public class GeminiService : IAIService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        EnsureApiKey(_settings.GeminiApiKey, "Gemini");

        using var client = _httpClientFactory.CreateClient();
        client.BaseAddress = new Uri("https://generativelanguage.googleapis.com/");
        client.Timeout = TimeSpan.FromSeconds(_settings.RequestTimeoutSeconds);

        var path = $"v1beta/models/{_settings.GeminiModel}:generateContent?key={Uri.EscapeDataString(_settings.GeminiApiKey)}";
        var body = new
        {
            contents = new[]
            {
